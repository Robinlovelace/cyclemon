...
Instead of requiring comprehensive origin-destination data, it can leverage single-destination data which is generally available with more recency from organizations keen to participate in specific-destination active travel planning efforts. The approach was originally developed in response to the needs of one such organization (Monmouthshire Council) holding data on journeys to their schools and leisure centres. Specific-destination models for cycling are derived from the PCT, while specific-destination models for walking are adapted by incorporating trip data into SpNA techniques. In addition to showing specific-destination models, however, network-only SpNA models are also employed to highlight integration of specific-destination routes with the wider active travel network, the importance of which is highlighted by Forsyth and Krizek (2011). 

Given the challenges of reliable prediction of walking and cycling mode choice and flows, the aim at this stage is not to produce calibrated predictions, but to visualize likely walking and cycling behaviours within settlements in a manner useful to the planning process. The approach maintains simplicity by deliberately eschewing recalibration: we transfer coefficients describing cyclist and pedestrian behaviour from other sources. The simplicity of the single-destination approach is a feature, not a bug; not only does it keep modelling costs low, but also allows significant transparency of modelling assumptions to our clients in the planning profession. Specifically, where models produced predictions that seemed to contradict local knowledge, this transparency made it relatively straightforward to for the client to determine whether this was (a) a fault in modelling assumptions, or (b) indicative of valid areas of concern in future network plans. That said, in most cases, modelling outputs showed strong alignment with networks previously planned on the basis of local knowledge.
