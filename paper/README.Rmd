...
<!-- We prefer remake Bernstein_2011 to interpret the makefile due to its enhanced debugging ability over standard GNU Make GNU Project - Free Software Foundation_2020 though it is also possible to build our models using the latter. -->
...
The SNA approach, on the other hand, made use of the 'Python stack', including `GeoPandas`, `shapely` and `pyrosm` [@gillies_shapely_2007; @jordahl_geopandas_2020; @tenkanen_pyrosm_2020].
...
A network processing step was scripted using QGIS [@qgisdevelopmentteam_qgis_2019] to snap downloaded pedestrian crossing points to the relevant nodes.
<!-- due to a bug in pyrosm; however we anticipate this will be resolved in the near future, removing the need for this step. -->
...
Metadata is automatically generated in json format alongside modelling outputs, based on the `view-metadata-template.jsontemplate` file.
...
3. QGIS project files containing all generated data displayed in the recommended manner, generated by the `create-qgis-project-from-metadata.py` script.
...
These figures are generated using the `create-qgis-cycle-summary.py` and `create-qgis-walk-summary.py` scripts.
<!-- Which figure? (RL) -->
